...
direction and its advance, so that its response comes
...
from math import pi
from track import Track, directions, xy_from_polar, Path
from graphics.polytrack import PolyTrack
...
track = path_track # Track(width=60, radius=180, center=Window.center, offset=(40, 0))
...
inner_poly = PolyTrack(points=track.contour['inner'])
outer_poly = PolyTrack(points=track.contour['outer'])
park.add_widget(inner_poly)
park.add_widget(outer_poly)
c = sq.Fauna.zoo[0]
park.add_widget(c)
c.pos = launch[:]
c.mov.pos = launch[:]
