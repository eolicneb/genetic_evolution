...
for iteration in range(1000):
    ...
    for ix, c in enumerate(sq.Fauna.zoo):
        ...
        for reg in c.mov.register[:-3:-1]:
            print('  {:12.0f} {:4d} d={:8.3f} t={:6.3f}'.format(*reg))
    ...
    for d in range(reproductions):
        sq.Fauna.zoo[darwin[d+reproductions][1]] = sq.Creature(movement=RegisteredMov(pos=[100,100], 
                                                                        box=Window, 
                                                                        brain=Brain(schema=schema, layers=sq.Fauna.zoo[darwin[d][1]].mov.brain.layers)))
